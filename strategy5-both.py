#not in use bc pretty bad, but is supposed to do negamax on the options left after heuristics
...
def legalMovesHeuristic(game, move):
    if move == 'X':
        movePos = {i for i in range(64) if game[i] == 'X'}
        otherPos = {i for i in range(64) if game[i] == 'O'}
    else:
    ...
    return returnMove(legal, game, move)
...
def negamax(board, token, levels):
    ...
    lm = legalMovesHeuristic(board, token)
    otherToken = 'X' if token == 'O' else 'O'
    if not lm:
