...
# .9040
# changed starting game strat to prioritize central pieces, get fewer pieces, no c pieces, corners, no edges
# .8952 w 20 games
...
def negamaxStart(brd, token ,improvable, hardBound, levels):
    if not levels: return [evalBoardStart(brd, token)]
    enemy = 'X' if token == 'O' else 'O'
    lm = legalMoves(brd, token)
    if not lm:
        lm = legalMoves(brd, enemy)
        if not lm: return [evalBoardStart(brd, token)] ### game over
        nm = negamaxStart(brd, enemy, -hardBound, -improvable, levels-1) + [-1]
        return [-nm[0]] + nm[1:]
    best = []
    newHB = -improvable
    for mv in lm:
        nm = negamaxStart(resultBoard(brd, token, mv, lm), enemy, -hardBound, newHB, levels-1) + [mv]
        if not best or nm[0]<newHB:
            best = nm
            if nm[0]<newHB:
                newHB = nm[0]
...
def main():
    ...
    if dots >=50:
        ...
        returnMove(set(posMoves), game, move)))  # returnMove(posMoves, game, move)))
        nm = negamaxStart(game, move, -65, 65, 7)
        print(nm, nm[-1])
    elif dots <= n:
        posMoves = legalMoves(game, move)
        print('Board:')
